...
1.3 การพัฒนาโปรแกรม Java

โปรแกรมได้รับการพัฒนาให้สามารถรับข้อมูลจากผู้ใช้ ตรวจสอบความถูกต้องของ Input และเปรียบเทียบการทำงานของ Algorithm A และ Algorithm B ภายในโปรแกรมเดียวกัน โดยมีการพัฒนาส่วนสำคัญดังนี้

ใช้ ArrayDeque เป็นโครงสร้าง Stack สำหรับ Algorithm B

กำหนดค่าเริ่มต้นของอาร์เรย์ผลลัพธ์เป็น -1 โดยใช้ Arrays.fill

เพิ่ม Input Validation สำหรับข้อมูลที่ไม่ใช่จำนวนเต็ม จำนวนอาคารน้อยกว่าหรือเท่ากับ 0 และความสูงติดลบ

รองรับ Edge Cases เช่น จำนวนอาคารไม่ถูกต้อง อาคารสูงเท่ากัน และกรณีที่ไม่พบอาคารที่สูงกว่า

วัดเวลาในการทำงานของแต่ละอัลกอริทึมด้วย System.nanoTime()

นับจำนวน Operation ของ Algorithm A และ Algorithm B

เปรียบเทียบผลลัพธ์ของทั้งสองวิธีและแสดง PASS เมื่อผลลัพธ์ตรงกัน

คำนวณ Largest Rectangle in Histogram เพิ่มเติมจากปัญหา Next Greater Element


1.4 รายละเอียดไฟล์ Source Code
...
เก็บชุดข้อมูล Input สำหรับการทดสอบหรือการทดลอง (ถ้ามี)

results/

เก็บผลการทดลอง ไฟล์ Excel และไฟล์ที่ใช้สำหรับสร้างกราฟ
...
เก็บรายงานฉบับ PDF และเอกสารประกอบที่เกี่ยวข้อง
...
1.8 การ Compile และ Run

Compile โปรแกรมหลัก

เปิด Terminal ที่โฟลเดอร์ Repository แล้วใช้คำสั่ง
...
1.9 ขนาดข้อมูลที่ใช้ในการทดลอง
...
ผลลัพธ์ของ Algorithm A และ Algorithm B ตรงกัน จึงแสดงสถานะ PASS


2. การวิเคราะห์ความซับซ้อน
...
Algorithm B ใช้ Stack ในการจัดเก็บ Index ของอาคารที่ยังไม่พบ Next Greater Element โดยแต่ละ Index ถูก Push อย่างมากหนึ่งครั้งและ Pop อย่างมากหนึ่งครั้งตลอดการทำงาน เมื่อวิเคราะห์ด้วย Amortized Analysis จึงมีการทำงานรวมเป็นเชิงเส้น
...
สามารถพัฒนาโปรแกรมเพิ่มเติมได้ดังนี้

เพิ่มการรองรับการนำเข้าข้อมูลจากไฟล์ เพื่อให้สามารถทดลองกับข้อมูลจำนวนมากได้สะดวกขึ้น

เพิ่มการแสดงสถานะของ Stack ในแต่ละขั้นตอน เพื่อช่วยให้เห็นการทำงานของ Monotonic Stack ได้ชัดเจนขึ้น

เพิ่มขนาดและจำนวนชุดข้อมูลในการทดลอง เพื่อให้สามารถวิเคราะห์ประสิทธิภาพได้ละเอียดมากขึ้น

เพิ่มการสรุปผลการทดลองและสร้างกราฟโดยอัตโนมัติ
...
ในเชิงอัลกอริทึม Algorithm A มี Time Complexity เป็น O(n²) ขณะที่ Algorithm B มี Time Complexity เป็น O(n) จากการวิเคราะห์แบบ Amortized Analysis โดยการทดลองจริงพบว่า Algorithm B ใช้จำนวน Operation น้อยกว่าเมื่อขนาดข้อมูลเพิ่มขึ้นอย่างชัดเจน ส่วนเวลาในการรันจริงอาจเปลี่ยนแปลงตามสภาพแวดล้อมของ JVM และเครื่องที่ใช้ทดลอง
